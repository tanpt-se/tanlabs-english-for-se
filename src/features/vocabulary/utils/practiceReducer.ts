import {
  VOCABULARY_COMPLETION_THRESHOLD,
...
import { gradeExercise, type LearnerResponse } from '@/features/vocabulary/utils/grade';
...
export function isCompletedScore(score: number): boolean {
  return score >= VOCABULARY_COMPLETION_THRESHOLD;
}
  ...
  switch (action.type) {
    case 'start': {
      ...
      return {
        ...
        situationSlug: action.situationSlug,
    ...
    case 'retry': {
      ...
      return {
        ...
        situationSlug: state.situationSlug,
...
export function buildCompletedSession(state: PracticeState) {
  ...
  return {
    ...
    itemResults: aggregateItemResults(state.exercises, state.checked),
